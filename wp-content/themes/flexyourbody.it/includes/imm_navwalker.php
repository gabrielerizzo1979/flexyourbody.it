    ...
    public function start_el(&$output, $item, $depth = 0, $args = null, $id = 0)
    {
      ...
      if ($this->has_children && 0 === $depth) {
        ...
        if ($depth > 0) {
          ...
          $atts['class'] = 'nav-link';
          if (!is_single() && (in_array('current-menu-item', $classes, true) || in_array('current-menu-parent', $classes, true))) {
            $atts['class'] = 'nav-link active';
          }


            if(is_tax()){ //Notice you can change the conditional from is_single() and $item->title
                $taxonomy = get_queried_object();
                $taxonomyName = $taxonomy->taxonomy;
                $postType = str_replace('category-','', $taxonomyName);

                switch ($postType){
                    case 'service':
                        if(strtolower($item->title) == 'servizi'){
                            $atts['class'] = 'nav-link active';
                        }
                        break;
                    case 'news':
                        if(strtolower($item->title) == 'novità'){
                            $atts['class'] = 'nav-link active';
                        }
                        break;
                    case 'document':
                        if(strtolower($item->title) == 'documenti e dati'){
                            $atts['class'] = 'nav-link active';
                        }
                        break;
                    case 'organization':
                        if(strtolower($item->title) == 'amministrazione'){
                            $atts['class'] = 'nav-link active';
                        }
                        break;
                }
